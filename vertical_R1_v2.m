...
for i=1:length(kappa)
    for ii=1:length(U)
        ...
        % Cruise 1 method
        cost1(count)=trapz(zHat_grid,(K(count).rM1-...
            ri1').^2);
        ...
        % Normal
        cost2(count)=trapz(zHat_grid,(K(count).rM2-...
            ri2').^2);
...
for j=2:size(I1,2)
    ...
    for i=1:length(K)
        count=count+1;
        % Cruise 1
        cost1E(count)=trapz(zHat_grid,(K(i).rM1-...
            r1_tmp').^2);
        
        %Cruise 2
        cost2E(count)=trapz(zHat_grid,(K(i).rM2-...
            r2_tmp').^2);
    end
...
for j=2:size(I1,2)
    ...
    % figure;hold on;
    % xlim([0 0.03])
    % plot(rS_tmp,zHat_grid,'r--');
    % plot(K(idxC1E).rM1,zHat_grid,'r');
    % plot(rD_tmp,zHat_grid,'b--');
    % plot(K(idxC2E).rM2,zHat_grid,'b');
    % pause
...
% cost fitting
cost1=trapz(zHat_grid,([K.rM2]-shallow_ri2').^2);
[~,CidxShallow2]=min(cost1);

cost2=trapz(zHat_grid,([K.rM2]-deep_ri2').^2);
[~,CidxDeep2]=min(cost2);
...
%%%%%%%%%%%%%%%%%%%%%
ax5=axes('color','none','position',[.625 0.52 0.075 0.125 ]); hold on;
line([0 0.8],[zStar_grid(sigIdx1) zStar_grid(sigIdx1)],...
    'color','k','linestyle','--','linewidth',0.75);
...
%%%%%%%%%%%%%%%
iax=axes('color','w','position',[0.165 0.42 0.15+0.165 0.2]); hold on;
grid on; axis ij;ylim([-125 125]); box on; xticks([]);

axV1=axes('color','w','position',[0.165 0.42 0.15 0.2 ]); hold on;
% %%%% CIOPS-E
...
%%%% Leg 1
l(1)=plot(K(Cidx1).rM1,zHat_grid,'color',rgb_x('light blue'),'linewidth',3);
...
%%%% Leg 2
l(2)=plot(K(Cidx2).rM2,zHat_grid,'-','color',rgb_x('dusty pink'),'linewidth',2);
l(2).Color=[l(2).Color 0.9];
...
%%%% Depth segregation %%%%%%
axV2=axes('color','w','position',[0.165+0.165 0.42 0.15 0.2]); hold on;
...
l1=line([0.0015 0.0055],[75;75],'linewidth',2);
l1.Color=[rgb_x('cerulean') 0.5];
l2=plot([0.0015 0.0055],[90 90],'linewidth',2);
...
text(0.006,75,'Interior    (\it{h}\rm{>279 m)}','fontsize',6,'color',...
    rgb_x('cerulean'));
text(0.006,90,'Boundary (\it{h}\rm{<279 m)}','fontsize',6,'color',...
    rgb_x('algae green'));

axes(axV1)

text(0.006,-100,'Leg 1','color','b','fontsize',6);
text(0.006,-85,'Leg 2','color','r','fontsize',6);

l1=line([0.0025 0.007],[75;75],'linestyle','--','color','k','linewidth',0.5);
l2=plot([0.0025 0.007],[90 90],'linewidth',2,'color','k');

text(0.008,75,'\it{r}','fontsize',6);
text(0.008,90,'1D model','fontsize',6);

% l=legend([l1 l2],'Interior\newline(\it{h} \rm{>279 m)}',...
%     'Boundary\newline(\it{h} \rm{>279 m)}','location','best','fontsize',6);

set(findall(gcf,'-property','fontname'),'fontname','Latin Modern Roman');

cf1=K(Cidx1).rM1;cf2=K(Cidx2).rM2;
cfIn1=K(CidxDeep1).rM1;cfIn2=K(CidxDeep2).rM2;
cfBn1=K(CidxShallow1).rM1;cfBn2=K(CidxShallow2).rM2;

save('/Users/samst/Library/CloudStorage/Dropbox/UBC/TReX_paper/code/letter/R1/vertObs.mat',...
    'ri1','ri2','zHat_grid','cf1','cf2','shallow_ri1','shallow_ri2',...
    'deep_ri1','deep_ri2','cfIn1','cfIn2','cfBn1','cfBn2');

%%
export_fig /Users/samst/Dropbox/UBC/TReX_paper/figures/R1/vertical_R1.pdf -dpdf -nofontswap
