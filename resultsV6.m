...
% m_annotation('textarrow',[dLon lonT1],[dLat latT1],'linewi',1,'color','k',...
%     'headlength',5);
s=m_scatter(dLon,dLat,70,'p','filled',...
    'markerfacecolor',[0.929,0.694,0.125],'markeredgecolor','k',...
    'linewidth',0.3);
m=max([T1.F.dTday]);
for i=1:length(T1.F)
    s=m_scatter(T1.F(i).resurfLon,T1.F(i).resurfLat,13-(T1.F(i).dTday/m*7)...
        ,'filled','markerfacecolor',rgb_x('off white'),'markeredgecolor','k',...
        'linewidth',0.4,'markerfacealpha',0.8-(T1.F(i).dTday/m*0.5));
end
m_scatter(stlon1(stBath1>-275),stlat1(stBath1>-275),5,'x','markeredgecolor',[.5 .5 .5],...
    'linewidth',0.3);
m_scatter(stlon1(stBath1<-275),stlat1(stBath1<-275),5,'^','markeredgecolor',[.5 .5 .5],...
    'linewidth',0.3);
...
m_ruler([0.8 0.975],0.125,'linewidth',0.5);
...
caxis(cax); 
m_gshhs_f('patch',col,'edgecolor',[0.3 0.3 0.3]);
s=m_scatter(dLon,dLat,50,'p','filled',...
    'markerfacecolor',[0.929,0.694,0.125],'markeredgecolor','k',...
    'linewidth',0.3);
...
m=max([T2.F.dTday]);
for i=1:length(T2.F)
    s=m_scatter(T2.F(i).resurfLon,T2.F(i).resurfLat,13-(T2.F(i).dTday/m*7),'filled',...
        'markerfacecolor',rgb_x('off white'),'markeredgecolor','k',...
        'linewidth',0.4,'markerfacealpha',0.8-(T2.F(i).dTday/m*0.5));
end
m_scatter(stlon2(stBath2>-275),stlat2(stBath2>-275),5,'x','markeredgecolor',[.5 .5 .5],...
    'linewidth',0.3);
m_scatter(stlon2(stBath2<-275),stlat2(stBath2<-275),5,'^','markeredgecolor',[.5 .5 .5],...
    'linewidth',0.3);
% m_scatter(CTD2(26).longitude(1),CTD2(26).latitude(1),4,'marker','^',...
...
scatter(vertcat(CTD1.TR_SF5),vertcat(CTD1.zStar),10,'filled',...
...
plot(Az_star1,zStar_grid,'color',[0.6 0.6 0.6]);
plot(Az_star1(1)*exp(-zStar_grid/200),zStar_grid,'--','color',[0.6 0.6 0.6]);
ylim([0 500]);yticks([]); xlim([0 max([Az_star1;Az_star2])])
xticklabels({'0','1×10^{11}','2×10^{11}'});xtickangle(0);
set(gca,'xaxislocation','top','ydir','reverse',...
    'xcolor',[0.6 0.6 0.6],'TickLength',[0.03, 0.01]);
...
l=line([0 0.9],[zStar_grid(sigIdx2) zStar_grid(sigIdx2)],...
    'color','k','linestyle',':','linewidth',1);
% l.Color=[l.Color 0.7];ll.Color=[ll.Color 0.7];

ax4i=axes('position',[0.8 0.525 0.15 0.425],'color','none'); hold on;
plot(Az_star2,zStar_grid,'color',[0.6 0.6 0.6]);
plot(Az_star2(1)*exp(-zStar_grid/200),zStar_grid,'--','color',[0.6 0.6 0.6]);
ylim([0 500]);yticks([]); xlim([0 max([Az_star1;Az_star2])])
xticklabels({'0','1×10^{11}','2×10^{11}'});xtickangle(0);
set(gca,'xaxislocation','top','ydir','reverse',...
    'xcolor',[0.6 0.6 0.6],'TickLength',[0.03, 0.01]);
...
%% 
% export_fig /Users/samst/Dropbox/UBC/TReX_paper/figures/letter/resultsV6.pdf -dpdf -nofontswap