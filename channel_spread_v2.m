...
yyaxis right
% plot(centredXM,ICmnM,'b:','linewidth',1);
% scatter(centredXM,ICmnM,30,'marker','s','markerfacecolor','b',...
%     'markeredgecolor','k','markerfacealpha',0.3);
l(3)=plot(X/1e3,cm(:,idxCM),'c','linewidth',0.4);
set(gca,'ycolor','k');
ylabel({'Relative tracer conc.'},'rotation',270);
ylim([0 1.2e-4]);
...
l(1)=plot(NaN,NaN,'k--');
plot(X/1e3,c1(:,idxC1),'w','linewidth',3);
l(2)=plot(X/1e3,c1(:,idxC1),'b-','linewidth',1.5);
plot(X/1e3,c2(:,idxC2),'w','linewidth',3);
...
xlim([0 525]);
text(15,37,'b)','fontsize',8);

set(findall(gcf,'-property','fontsize'),'fontsize',8);
set(findall(gcf,'-property','fontname'),'fontname','Latin Modern Roman');

% 
% legend(l,'Overlapping window mean',...
%     sprintf('Leg 1 1D model: k=%3.0f m^2 s^{-1}, U=%2.1f cm s^{-1}',...
%     A(idxs(1,idxC1)),U(idxs(2,idxC1))*100),...
%     sprintf('Leg 2 1D model: k=%3.0f m^2 s^{-1}, U=%2.1f cm s^{-1}',...
%     A(idxs(1,idxC2)),U(idxs(2,idxC2))*100),'location','northeast',...
%     'fontsize',6);
...
ax3=axes('position',[.585 .2 0.15 0.7]); 
...
ax4=axes('position',[.79 .2 0.2 0.7]);
hold on
...
%%
export_fig /Users/samst/Dropbox/UBC/TReX_paper/figures/letter/channelV6.pdf -dpdf -nofontswap
