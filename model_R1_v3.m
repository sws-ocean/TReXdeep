...
for i=1:2
    ...
    idx3=find(inShape(LC.station_bound,MT.(flds{i}).lon,MT.(flds{i}).lat) & model_z<-200);
...
%% errors
B1=bootrnd(size(centredXM,2),100);
B2=bootrnd(size(centredXM,2),100);
    ...
    for i=1:length(A)
        for ii=1:length(U)
            ...
            c1E(:,count)=(c1E(:,count)-min(c1E(:,count)))/...
                (max(c1E(:,count))-min(c1E(:,count))).*max(MT.L1.ICmnM(B1(:,j)));

            c2E(:,count)=M0./sqrt(4*pi*A(i)*t(2)).*exp( -(X-U(ii)*t(2)).^2./(4*A(i)*t(2)));%...
            c2E(:,count)=(c2E(:,count)-min(c2E(:,count)))/...
                (max(c2E(:,count))-min(c2E(:,count))).*max(MT.L2.ICmnM(B2(:,j)));
...
% order of magnitude larger than observations, suggesting that vertical
